...
public final class SyncTaskScheduler {
    ...
    public SyncTaskScheduler(final SyncerApplication application) {
    ...
    private void setup() {
        for (final AbstractSyncTaskConfiguration configuration : application.getConfiguration().getTasks()) {
            try {
                ...
                final Constructor<?> constructor = taskClass.getDeclaredConstructor(SyncerApplication.class, configuration.getClass());

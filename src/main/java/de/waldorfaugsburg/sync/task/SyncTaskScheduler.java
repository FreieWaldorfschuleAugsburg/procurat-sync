...
public final class SyncTaskScheduler {
    ...
    public SyncTaskScheduler(final SyncApplication application) {
    ...
    private void setup() {
        for (final AbstractSyncTaskConfiguration configuration : application.getConfiguration().getTasks()) {
            try {
                ...
                final Constructor<?> constructor = taskClass.getDeclaredConstructor(SyncApplication.class, configuration.getClass());

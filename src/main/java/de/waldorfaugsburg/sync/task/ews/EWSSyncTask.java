package de.waldorfaugsburg.sync.task.ews;
...
import de.waldorfaugsburg.sync.client.ews.EWSClient;
import de.waldorfaugsburg.sync.client.procurat.ProcuratClient;
import de.waldorfaugsburg.sync.module.procurat.model.*;
...
public final class EWSSyncTask extends AbstractSyncTask<EWSSyncTaskConfiguration> {
    ...
    public EWSSyncTask(final SyncApplication application, final EWSSyncTaskConfiguration configuration) {
    ...
    public void run() throws Exception {
        ...
        for (final EWSSyncTaskConfiguration.ContactGroup group : selectorGroupMap.keySet()) {
            ...
            for (final EWSSyncTaskConfiguration.Selector selector : selectors) {
                ...
                // Create contact if not yet existent
                if (contact == null) {
                    contact = createContact(procuratClient, ewsClient, rootGroupMemberships, person);
                    // Skip contact if creation fails
                    if (contact == null) {
                        recordDeviation("Could not create contact '%s' (Id: %s)", person.getLastName() + " " + person.getFirstName(), person.getId());
                        continue;
                    }
